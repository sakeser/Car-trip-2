...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_14_15 = object : Migration(14, 15) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE `trips` ADD COLUMN `gnssAvgSatsUsed` REAL NOT NULL DEFAULT 0")
                database.execSQL("ALTER TABLE `trips` ADD COLUMN `gnssAvgCn0` REAL NOT NULL DEFAULT 0")
        ...
        // Manual drive/non-drive override. Nullable (NULL = auto-detect), so existing rows stay on auto.
        ...
        // v20: magnitude-weighted speeding severity (Rev BF) — basis of the new Safety speeding penalty.
        private val MIGRATION_19_20 = object : Migration(19, 20) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE `trips` ADD COLUMN `speedingSeverity` REAL NOT NULL DEFAULT 0")
            }
        }

        // v21: drawdowns (Rev CI) — count + severity of forced cruise-then-slow-then-recover events.
